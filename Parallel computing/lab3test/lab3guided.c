...
int main(int argc, char *argv[]) {
    ...
    double *restrict arr2Coppy = malloc(M2  * sizeof(double));
        ...
        omp_set_schedule(omp_sched_guided, N);
        omp_set_dynamic(0);
    ...
    for(unsigned int k = 0; k < 100; k++) {
        ...
        #pragma omp parallel for default(none) shared(M2, arr2, arr2Coppy, A, seed) schedule(guided, N) firstprivate(N)
        {
        ...
        #pragma omp parallel for default(none) shared(M1, arr1, A, seed) schedule(guided, N) firstprivate(N)
        {
        ...
        #pragma omp parallel for default(none) shared(M2, arr1, arr2, A, seed) schedule(guided, N) firstprivate(N)
        {   
        ...
        double min = arr2[0];
        #pragma omp parallel for default(none) shared(M2, arr2, min, A, seed) reduction(+:sum) schedule(guided, N) firstprivate(N)
