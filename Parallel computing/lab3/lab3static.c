...
int main(int argc, char *argv[]) {
    ...
    for(unsigned int k = 0; k < 100; k++) {
        ...
        #pragma omp parallel for default(none) shared(M1, arr1, A, seed) schedule(static, N) firstprivate(N)
        {
        ...
        #pragma omp parallel for default(none) shared(M2, arr1, arr2, A, seed) schedule(static, N) firstprivate(N)
        {   
        ...
        double min = arr2[0];
        #pragma omp parallel for default(none) shared(M2, arr2, min, A, seed) reduction(+:sum) schedule(static, N) firstprivate(N)
